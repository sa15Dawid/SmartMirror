...
if __name__ == '__main__':
    ...
    timer = QTimer()
    timer.timeout.connect(ui.getWeatherIcon)
    timer.timeout.connect(ui.getWeather)
    timer.timeout.connect(ui.getCountry)
    timer.timeout.connect(ui.getCity)
    timer.timeout.connect(ui.getSunrise)
    timer.timeout.connect(ui.getSunset)
    timer.timeout.connect(ui.getTempMin)
    timer.timeout.connect(ui.getTempMax)
    timer.timeout.connect(ui.getDescription)
    timer.start(600000)
